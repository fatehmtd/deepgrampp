...
{
public:
    STTClient(const std::string &host, const std::string &apiKey, const std::string &port = "443")
    ...
    void startKeepalive()
    {
        keepaliveThread_ = std::thread([this]()
        {
            ...
            while (keepReceiving_ && ws_.is_open())
            {
                ...
                {
                    beast::error_code ec;
                    ws_.write(net::buffer(deepgram::control::KEEPALIVE_MESSAGE), ec);
...
private:
    ...
    void sendCloseStream()
    {
        try
        {
            ...
            ws_.binary(false);
            beast::error_code ec;
            ws_.write(net::buffer(deepgram::control::CLOSE_MESSAGE), ec);
    ...
    void handleResponse(const std::string &message)
    {
        try
        {
            ...
            deepgram::TranscriptionResult::fromJson(json).print();
            std::cout<< "--------------" << std::endl;
            return;
            ...
            if (json.contains("type"))
            {
                ...
                if (type == "Metadata")
                {
                    ...
                    std::cout << "Request ID: " << json.value("request_id", "unknown") << std::endl;
                    std::cout << "Duration: " << json.value("duration", 0.0) << "s" << std::endl;
                    std::cout << "Channels: " << json.value("channels", 0) << std::endl;
                    std::cout << "==============================\n" << std::endl;
                    
                    // Start keepalive after receiving metadata
                    startKeepalive();
                }
                else if (type == "Results")
                {
                    if (json.contains("channel") && 
                        json["channel"].contains("alternatives") && 
                        !json["channel"]["alternatives"].empty())
                    {
                        auto alternative = json["channel"]["alternatives"][0];
                        if (alternative.contains("transcript"))
                        {
                            std::string transcript = alternative["transcript"];
                            bool isFinal = json.value("is_final", false);
                            
                            if (!transcript.empty())
                            {
                                std::cout << (isFinal ? "🔵 FINAL: " : "⚪ INTERIM: ") 
                                         << "\"" << transcript << "\"" << std::endl;
                            }
                        }
                    }
                ...
                {
                    std::cout << "🔚 Utterance ended" << std::endl;
                }
                ...
                {
                    std::cout << "📨 " << type << " message received" << std::endl;
                }
            }
        }
        catch (const std::exception &e)
...
int main()
{
    try
    {
        ...
        STTClient client("api.deepgram.com", apiKey);
